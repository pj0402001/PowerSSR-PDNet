...
\begin{abstract}
% ─────────────────────────────────────────────────────────────────────────────
We study static security region (SSR) learning in the correct \emph{generator power space}, where the multi-solution Bukhsh benchmark cases exhibit disconnected and highly nonconvex secure sets that are expensive to obtain by dense nonlinear feasibility scanning. We propose an enhanced physics-informed framework centered on SSR-PDNet and an energy-closure full-state surrogate (EC-PDNet), which jointly predicts security membership and internal operating states while embedding active-power balance into the network parameterization. Across WB2, WB5, LMBM3, and case9mod, the method preserves the benchmark SSR topology and delivers high classification accuracy (e.g., F1=0.9671 on WB5 and F1=0.9716 on case9mod), while the structured full-state model substantially improves slack-power fidelity on case9mod (test $P_{G1}$ MAE reduced from 0.8595 MW to 0.1304 MW) and lowers overall state error, demonstrating a practical path from boundary classification toward fast, physics-consistent surrogate replacement of pointwise traditional solves.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Physics-structured full-state surrogate.} Beyond boundary classification, we introduce an energy-closure parameterization (EC-PDNet) that embeds active-power balance into network outputs and jointly predicts security and internal OPF states.

  \item \textbf{Benchmark-level evidence and open reproducibility.} On WB2/WB5/LMBM3/case9mod we show accurate topology recovery and strong quantitative performance, with marked slack-power improvement on case9mod (0.8595\,$\rightarrow$\,0.1304 MW MAE), and release code, pipelines, and artifacts for full reproduction.
\end{enumerate}
