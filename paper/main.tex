...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{case9mod_dl_comparison.png}
  \caption{case9mod DL-vs-traditional comparison in generator power space
    $(P_{G2}, P_{G3})$ using EC-PDNet+WLDG-BE. Panel (a) is the traditional
    IPOPT reference; panel (b) is the learned security-score field with
    predicted boundary and true boundary overlays; panel (c) is the
    disagreement map. Axes start from zero to explicitly visualize the
    $[0,10)$~MW guard-band infeasible area.}
...
\begin{figure}[!t]
  ...
  \caption{case9mod local zoom around a boundary-dense region under
    EC-PDNet+WLDG-BE. Panel (a) shows local secure/insecure layout; panel (b)
    shows density heterogeneity via hexbin counts; panel (c) overlays the
    predicted boundary (white dashed) and traditional boundary (black),
    highlighting the transition corridor near the true SRB.}
...
\begin{table}[!t]
  ...
  \begin{tabular}{llccc}
    ...
    \multirow{3}{*}{case9mod}
      ...
      & EC-PDNet+WLDG-BE & \textbf{0.9979} & 0.4433 & n/a \\
    \bottomrule
  \end{tabular}
\end{table}
...
In addition, the regenerated comparison figure Fig.~\ref{fig:case9mod_ssr}
